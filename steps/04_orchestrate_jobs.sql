...
create or alter table vacation_spots (
  ...
  , aquarium_cnt int   -- Added
  , zoo_cnt int        -- Added
  , korean_restaurant_cnt int    -- Added
...
create or alter task vacation_spots_update
  ...
  AS MERGE INTO vacation_spots USING (
    ...
    join silver.attractions att on att.geo_name = city.geo_name  -- Added
    -- STEP 5: INSERT CHANGES HERE
    ...
    begin
      ...
      if (:options = '[]') then
        CALL SYSTEM$SEND_EMAIL(
            'email_integration',
            'xiejing23@gmail.com', -- INSERT YOUR EMAIL HERE
